 ...
 *
 * The drop only tells the live component which row moved where; it reorders the submitted
 * values and re-renders. A reordered DOM alone would look right until the next re-render —
 * editing a price, adding a tax — and then silently snap back.
 *
 * The same move is on the arrow keys, so the handle is a real control and not just a grip
 * that happens to take focus.
 */
/* stimulusFetch: 'lazy' */
export default class extends Controller<HTMLElement> {
    ...
    /*
     * Listened for on the list rather than bound to each handle, so a row added after this
     * controller connected is covered too.
     */
    keydown(event: KeyboardEvent) {
        ...
        // Before the bounds check: an arrow key on the first or last handle must do nothing,
        // and scrolling the page out from under it is not nothing.
    ...
    private async move(from: number, to: number) {
        ...
        // The re-render replaces the row the key press came from, so focus would land back on
        // the document. Put it on the moved line's handle, ready for the next press.
    ...
    /*
     * The handle's accessible name is unchanged by the move, so a screen reader is told
     * nothing about what happened. Where the line landed is the whole outcome, so say it.
     */
    ...
    /*
     * Outside the component: it re-renders the row list on every move, and a region morphed
     * away mid-announcement is not read.
     */
