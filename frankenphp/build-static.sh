...
if [ -z "${SOLIDINVOICE_VERSION}" ]; then
	SOLIDINVOICE_VERSION="$(git rev-parse --verify HEAD)"
	export SOLIDINVOICE_VERSION
elif [ -d ".git/" ]; then
	CURRENT_REF="$(git rev-parse --abbrev-ref HEAD)"
	export CURRENT_REF

	if echo "${SOLIDINVOICE_VERSION}" | grep -F -q "."; then
		# Tag

		# Trim "v" prefix if any
		SOLIDINVOICE_VERSION=${SOLIDINVOICE_VERSION#v}
		export SOLIDINVOICE_VERSION

		git checkout "v${SOLIDINVOICE_VERSION}"
	else
		git checkout "${SOLIDINVOICE_VERSION}"
	fi
fi
...
if [ -f dist/cache_key ] && [ "$(cat dist/cache_key)" = "${cache_key}" ] && [ -f "dist/static-php-cli/buildroot/lib/libphp.a" ]; then
	...
	${spcCommand} download --with-php="${PHP_VERSION}" --for-extensions="${PHP_EXTENSIONS}" --for-libs="${PHP_EXTENSION_LIBS}" --ignore-cache-sources=php-src --prefer-pre-built --debug -vvv
	# shellcheck disable=SC2086
...
CGO_LDFLAGS="${CGO_LDFLAGS} ${PWD}/buildroot/lib/libbrotlicommon.a ${PWD}/buildroot/lib/libbrotlienc.a ${PWD}/buildroot/lib/libbrotlidec.a $(${spcCommand} spc-config "${PHP_EXTENSIONS}" --with-libs="${PHP_EXTENSION_LIBS}" --libs)"
if [ "${os}" = "linux" ]; then
...
# shellcheck disable=SC2086
CGO_ENABLED=1 \
	XCADDY_GO_BUILD_FLAGS="-buildmode=pie -tags nowatcher,cgo,netgo,osusergo,static_build,nobadger,nomysql,nopgx -ldflags \"-linkmode=external -extldflags '-static-pie ${extraExtldflags}' ${extraLdflags} -X 'github.com/caddyserver/caddy/v2.CustomVersion=SolidInvoice ${SOLIDINVOICE_VERSION} PHP ${LIBPHP_VERSION} Caddy'\"" \
