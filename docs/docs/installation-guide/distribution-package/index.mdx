...
SolidInvoice uses a single background worker for both async messages (emails, search indexing, webhook dispatch) and scheduled tasks (recurring invoices, reminders, overdue checks). The worker runs `bin/console messenger:consume --all` in a loop.

Follow the [cron job setup guide](./cron-job-setup.md) — it covers systemd, cron, cPanel, Plesk, and Windows Task Scheduler.

:::warning
Without the worker running, async features and scheduled tasks won't fire — emails won't send, recurring invoices won't generate, and search indexing won't update.
:::
